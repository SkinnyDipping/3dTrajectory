...
void PointCloudPreview::renderFrame(Point3D *frame, int width, int height, cv::Mat_<float> matrix)
{
    frame_buffer.create();
    frame_buffer.bind();
    frame_buffer.allocate(frame, width*height*sizeof(Point3D));
...
{
    points_buffer.bind();
    Point3D* pts = new Point3D[n+3];
    pts = points;
    pts[n] = Point3D(10,0,0);
    pts[n+1] = Point3D(10,0,10);
    pts[n+2] = Point3D(0,10,0);
    points_buffer.allocate(pts, (n+3)*sizeof(Point3D));
