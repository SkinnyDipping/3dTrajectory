...
//TODO: ZROBIĆ TO KURTWA
Point3D multiply(cv::Mat matrix, Point3D point) {
	assert(matrix.cols == 4);
	Point3D output = Point3D();
	cv::Mat_<double> m = matrix;
	output.x = m(0, 0) * point.x + m(0, 1) * point.y + m(0, 2) * point.z;
	output.y = m(1, 0) * point.x + m(1, 1) * point.y + m(1, 2) * point.z;
	output.z = m(2, 0) * point.x + m(2, 1) * point.y + m(2, 2) * point.z;
	return output;
}
...
Point3D Quaternion::rotate(Point3D point, Quaternion q, Point3D finalCentroid) {
...
//#define ROTATE_WITH_MATRIX
#ifdef ROTATE_WITH_MATRIX
std::vector<Point3D>& Quaternion::rotate(std::vector<Point3D>& points,
		Quaternion q, Point3D finalCentroid) {
	...
	cv::Mat transformationMatrix = q.toTransformationMatrix();
	...
	for (int i = 0; i < points.size(); i++) {
		...
		//Rotation
		Point3D rotated = multiply(transformationMatrix, points[i]);
